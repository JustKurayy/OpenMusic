...
import { Heart, SkipBack, SkipForward, Play, Pause, Repeat, Shuffle, Volume2, List, Monitor, Mic2, PictureInPicture, Maximize2, VolumeX, Volume1 } from "lucide-react";
...
export default function MusicPlayer({ onToggleQueue }: { onToggleQueue?: () => void }) {
  const {
    currentTrack,
    isPlaying,
    currentTime,
      ...
      <footer className="bg-black px-4 py-3 h-24 flex items-center">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center space-x-4 flex-1 min-w-0 max-w-sm">
            ...
            <Button variant="ghost" size="sm" className="text-gray-500 cursor-not-allowed" disabled>
          ...
          <div className="flex flex-col items-center space-y-2 flex-1 max-w-2xl">
            <div className="flex items-center space-x-2">
              <Button variant="ghost" size="sm" className="text-gray-400 opacity-50 cursor-not-allowed" disabled>
                <Shuffle className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="sm" className="text-gray-400 opacity-50 cursor-not-allowed" disabled>
                <SkipBack className="w-5 h-5" />
              </Button>
              <Button variant="ghost" size="sm" className="w-8 h-8 bg-white text-black rounded-full opacity-50 cursor-not-allowed" disabled>
          ...
          <div className="flex items-center space-x-1 flex-1 justify-end max-w-sm">
            ...
            <div className="flex items-center space-x-2 group">
              ...
              <div className="w-24">
                <Slider value={[0]} max={100} step={1} disabled className="[&_.slider-track]:bg-gray-600 [&_.slider-range]:bg-white [&_.slider-thumb]:bg-white" />
              </div>
            </div>
          </div>
        </div>
    ...
    <footer className="bg-black px-4 py-3 h-24 flex items-center">
      <div className="flex items-center justify-between w-full">
        ...
        <div className="flex items-center space-x-4 flex-1 min-w-0 max-w-sm">
          {isRadio ? (
            <div className="relative group cursor-pointer">
              <img
                src={currentRadio.favicon || `https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=56&h=56&fit=crop`}
                alt="Radio logo"
                className="w-14 h-14 rounded-md object-cover shadow-lg bg-white"
                onError={e => { e.currentTarget.src = `https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=56&h=56&fit=crop`; }}
              />
        ...
        <div className="flex flex-col items-center space-y-2 flex-1 max-w-2xl">
          <div className="flex items-center space-x-2">
            {isRadio ? (
              ...
              <>
                ...
                >
                  <Shuffle className="w-4 h-4" />
                  {isShuffled && <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-green-500 rounded-full"></div>}
                </Button>
                <Button 
                  variant="ghost" 
                  size="sm" 
                ...
                >
                  ...
                  {repeatMode > 0 && <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-green-500 rounded-full"></div>}
                </Button>
              </>
            )}
          </div>
          ...
          {isRadio ? (
            <div className="flex items-center space-x-2 w-full max-w-md">
              <span className="text-xs text-gray-400 w-10 text-right">Live</span>
              <div className="flex-1 h-1 bg-gray-700 rounded-full">
                <div className="h-1 bg-green-500 rounded-full w-full"></div>
              </div>
              <span className="text-xs text-gray-400 w-10">Radio</span>
            </div>
          ) : (
            <div className="flex items-center space-x-2 w-full max-w-md">
