...
export default function MusicPlayer({ onToggleQueue }: { onToggleQueue?: () => void }) {
    const {
        currentTrack, isPlaying, currentTime, duration, volume, toggle,
        seekTo, setVolume, next, previous, showLyrics, toggleLyrics,
        currentRadio, isRadioPlaying, toggleRadio,
    } = usePlayer();
    ...
    const VolumeIcon = (!isMuted && volume > 0) ? (volume < 0.5 ? Volume1 : Volume2) : VolumeX;
    ...
    return (
        <footer className="px-4 py-3 h-24 flex items-center">
            <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center space-x-4 flex-1 min-w-0 max-w-sm">
                    <img
                        src={isRadio ? currentRadio.favicon : (currentTrack?.coverArt ? tracksApi.getArtworkUrl(currentTrack.id) : "")}
                        alt="Cover"
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-[#b3b3b3] mt-0.5 truncate hover:underline hover:text-white cursor-pointer">
                            {isRadio ? currentRadio.country : currentTrack?.artist}
                        </p>
                    </div>
                    <Button 
                ...
                <div className="flex flex-col items-center space-y-2 flex-1 max-w-2xl">
                    <div className="flex items-center space-x-6">
                        ...
                        <Button variant="ghost" size="sm" className="text-[#b3b3b3] hover:text-white hover:bg-transparent" onClick={previous}>
                            <SkipBackIcon />
                        </Button>
                        <Button
                            variant="ghost"
                        ...
                        >
                            {(isRadio ? isRadioPlaying : isPlaying) ? <PauseIcon /> : <PlayIcon />}
                        </Button>
                        <Button variant="ghost" size="sm" className="text-[#b3b3b3] hover:text-white hover:bg-transparent" onClick={next}>
                            <SkipForwardIcon />
                        </Button>
                        <Button 
                            variant="ghost" size="sm" 
                        ...
                        >
                            <Repeat className={`w-4 h-4 ${repeatMode > 0 ? "fill-current" : ""}`} />
                        </Button>
                    </div>
                ...
                <div className="flex items-center space-x-3 flex-1 justify-end max-w-sm">
                    ...
                    <Button variant="ghost" size="sm" className="text-[#b3b3b3] hover:text-white hover:bg-transparent" onClick={onToggleQueue}>
                        <List className="w-4 h-4" />
                    </Button>
                    <div className="flex items-center space-x-2 w-32">
                        <Button variant="ghost" size="sm" onClick={toggleMute} className="text-[#b3b3b3] hover:text-white hover:bg-transparent p-0">
