import { 
  ...
  type Track, type InsertTrack, type TrackWithUser,
  type Playlist, type InsertPlaylist, type PlaylistWithTracks, type PlaylistWithUser,
...
export interface IStorage {
  ...
  updatePlaylist(id: number, playlist: Partial<InsertPlaylist>, userId: number): Promise<Playlist | undefined>;
  ...
  removeTrackFromPlaylist(playlistId: number, trackId: number, userId: number): Promise<boolean>;
  reorderPlaylistTracks(playlistId: number, trackIds: number[], userId: number): Promise<boolean>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async searchTracks(query: string, userId?: number): Promise<TrackWithUser[]> {
  ...
  async updatePlaylist(id: number, playlist: Partial<InsertPlaylist>, userId: number): Promise<Playlist | undefined> {
  ...
  async addTrackToPlaylist(playlistTrack: InsertPlaylistTrack): Promise<PlaylistTrack> {
    const [newPlaylistTrack] = await db.insert(playlistTracks).values(playlistTrack).returning();
    return newPlaylistTrack;
  }

  async removeTrackFromPlaylist(playlistId: number, trackId: number, userId: number): Promise<boolean> {
  ...
  async reorderPlaylistTracks(playlistId: number, trackIds: number[], userId: number): Promise<boolean> {
    // First verify the user owns the playlist
    const [playlist] = await db.select().from(playlists).where(
      and(eq(playlists.id, playlistId), eq(playlists.userId, userId))
    );
